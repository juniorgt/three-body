...
class BodySystem:
    ...
    def _generate_fun(self, masses, G, nBodies=3):
        ...
        def fun(t, y):  # Constante de gravitación universal
            ...
            body_states = y.reshape((nBodies, n_dim * n_variable))

            # Extract positions and velocities separately
            positions, velocities = body_states[:, ::2], body_states[:, 1::2]

            # Calculate distances
            position_differences = positions[:, None, :] - positions
            cubic_distances = np.linalg.norm(position_differences, axis=2) ** 3

            # Calculate accelerations using Newton's laws with masses
            forces = G * (masses[:, None] * position_differences)
            accelerations = np.sum(forces, axis=0) / cubic_distances
