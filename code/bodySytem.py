...
class BodySystem:
    ...
    def _create_save_directories(self):
        self.save_route_images = os.path.join(
            save_route_images,
            f"{self.name}_{self.h}",
            f"{self.ODESolver}_{self.method}",
        )
        self.save_route_data = os.path.join(
            save_route_data, f"{self.name}_{self.h}", f"{self.ODESolver}_{self.method}"
        )
        os.makedirs(self.save_route_images, exist_ok=True)
        os.makedirs(self.save_route_data, exist_ok=True)

        file_name_data = f"{self.name}_{self.h}_{self.ODESolver}_{self.method}"
        self.path_data = os.path.join(self.save_route_data, file_name_data)

        file_name_toml = f"{self.name}_{self.h}_{self.ODESolver}_{self.method}.toml"
        self.file_path_toml = os.path.join(self.save_route_data, file_name_toml)
    ...
    def save_orbit_figure(self, route=None):
        ...
        filename = f"{self.name}_{self.h}_{self.ODESolver}_{self.method}.png"
    ...
    def save_energy_figure(self, route=None):
        ...
        filename = f"{self.name}_{self.h}_{self.ODESolver}_{self.method}_energy.png"
        filepath = os.path.join(route, filename)
        fig.savefig(filepath)
        plt.close(fig)

        fig, _ = self.generate_energy_figure(True, alpha=0.5)
        filename = f"{self.name}_{self.h}_{self.ODESolver}_{self.method}_energy_t.png"
        filepath = os.path.join(route, filename)
        fig.savefig(filepath)
        plt.close(fig)
    ...
    def save_total_energy_figure(self, route=None):
        ...
        filename = (
            f"{self.name}_{self.h}_{self.ODESolver}_{self.method}_total_energy.png"
        )
        filepath = os.path.join(route, filename)
        fig.savefig(filepath)
        plt.close(fig)
    ...
    def save_angular_momentum_figure(self, route=None):
        ...
        filename = (
            f"{self.name}_{self.h}_{self.ODESolver}_{self.method}_angular_momentum.png"
        )
        filepath = os.path.join(route, filename)
        fig.savefig(filepath)
        plt.close(fig)
    ...
    def save_linear_momentum_figure(self, route=None):
        ...
        filename = (
            f"{self.name}_{self.h}_{self.ODESolver}_{self.method}_linear_momentum.png"
        )
        filepath = os.path.join(route, filename)
        fig.savefig(filepath)
        plt.close(fig)

    def _error(self, data: np.ndarray):
        reference_value = data[0]
        absolute_errors = np.abs(data[1:] - reference_value)
        if np.abs(reference_value) <= EPS:
...
if "__main__" == __name__:
    ...
    bdeu = BodySystem(init_setup=init_setup, ODESolver="rk", method="euler")
    bdeu.run_simulation()
    # bdrk.load_simulation()
    # t, y = bdrk.run_simulation()
    bdeu.plot()
